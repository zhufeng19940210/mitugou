...
{
    if (!_titleArray) {
        _titleArray = [NSMutableArray arrayWithObjects:@"个人信息认证",@"活体检测认证",@"运营商认证",@"淘宝认证",@"央行认证", nil];
    }
    return _titleArray;
}
- (void)viewDidLoad {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        //运营商认证
        OperatorVC *opearvc = [[OperatorVC alloc]init];
        [self.navigationController pushViewController:opearvc animated:YES];
        //_sdkFunction  = LMZXSDKFunctionMobileCarrie;
        //_functionType = @"mobile";
        //[self pushToLimuSDKMethod];
    }else if(indexPath.section == 3){
        //淘宝认证
        TaobaoVC *taobaovc = [[TaobaoVC alloc]init];
       ...
       // _sdkFunction  =  LMZXSDKFunctionCentralBank;
        ...
        //[self pushToLimuSDKMethod];
        CentralBankCreditVC *bankvc = [[CentralBankCreditVC alloc]init];
